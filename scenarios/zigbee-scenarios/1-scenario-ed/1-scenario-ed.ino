// ZigBee End Device with DHT22
// Reads temperature/humidity, sends to coordinator, then sleeps
...
void readDHT22() {
  ...
  if (!success) {
    ...
    Serial.println("1. Check if DHT22 is properly connected to power (3.3V and GND)");
    Serial.println("2. Verify the data pin is connected to GPIO4 and has a pull-up resistor (4.7k-10k ohm)");
    Serial.println("3. Try powering the DHT22 externally with a stable power source");
    Serial.println("4. Make sure cable length is not too long (<20cm recommended)");
...
void basicSetup() {
  ...
  Serial.println("Stage 1: Basic Setup Complete.");
  delay(2000);
}
...
bool zigbeeInitialize() {
  ...
  Serial.println("Stage 2: Starting Zigbee...");
  if (!Zigbee.begin()) {
...
void normalOperation() {
  Serial.println("Stage 3: Normal Operation.");
